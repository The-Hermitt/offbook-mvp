...
export async function resumeSharedAudioContext() {
  try {
    const ac = typeof getSharedAudioContext === "function" ? getSharedAudioContext() : null;
    if (!ac) return false;
    if (ac.state !== "running") {
      await ac.resume();
    }
    // iOS unlock: brief silent buffer to re-arm audio output
    ...
    src.start(0);
    // ensure GC
    setTimeout(() => { try { src.disconnect(); } catch {} }, 0);
